...
@section('content')
                ...
                @if (count($timeSheets) > 0)
                    @foreach ($timeSheets as $date => $logs)
                                    ...
                                    <tr>
                                        ...
                                        <td style="width: 10%;">
                                            @if ($timeSheet['billable'] == 'Yes')
                                                <img src="{{ URL::asset('public/assets/images/icons/accept.png') }}"
                                                    alt="Yes" height="20" width="20" style="margin-left: 20px;">
                                            @else
                                                <img src="{{ URL::asset('public/assets/images/icons/reject.png') }}"
                                            ...
                                            <td class="Action">
                                                <div class="action-btn bg-info ms-2">
                                                    <a href="#"
                                                        ...
                                                        data-url="{{ route('task.team.timesheet.edit', [$timeSheet['user_id'], $timeSheet['id']]) }}"
                        ...
                        <p class="my-3" style="text-align: right"><strong>Total: </strong>
                            {{ !empty($finalTime[$date]) ? floor($finalTime[$date] / 60) . 'h ' : '' }}
                            {{ $finalTime[$date] - floor($finalTime[$date] / 60) * 60 > 0 ? $finalTime[$date] - floor($finalTime[$date] / 60) * 60 . 'm' : '' }}
                            <strong> Billable Time: </strong>
                            {{ !empty($totalBillableTimeSum[$date]) ? floor($totalBillableTimeSum[$date] / 60) . 'h ' : '' }}
                            {{ isset($totalBillableTimeSum[$date]) ? ($totalBillableTimeSum[$date] - floor($totalBillableTimeSum[$date] / 60) * 60 > 0 ? $totalBillableTimeSum[$date] - floor($totalBillableTimeSum[$date] / 60) * 60 . 'm' : '') : '0h' }}
                        </p>
                    @endforeach
                @else
    ...
    @if ($showMoreButtonStatus)
        <button type="button" class="btn btn-primary my-2" data-endDate="" data-lastDate="{{ $lastDate }}" id="teamShowMore">Show
            More</button>
    @endif
    <!-- Sidebar -->
...
@push('script-page')
    <script>
        $(function() {
            ...
            var config = {
                routes: {
                    filter: "{{ route('teamtimesheet.entries.filter') }}"
                }
            };
