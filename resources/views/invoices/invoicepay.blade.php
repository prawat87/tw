...
@push('script-page')
    ...
    @if( $invoice->getDue() > 0 && $settings['site_enable_stripe'] == 'on')
...
@section('action-button')
    <div class="row align-items-center m-1">
        ...
        @endif
        <div class="col-auto pe-0">
            <button type="button" class="btn btn-print-invoice  btn-primary"> 
                <a href="{{ route('get.invoice',Crypt::encrypt($invoice->id)) }}" class="text_white" title="{{__('Print Invoice')}}"><span class="text-white"><i class="ti ti-printer text-white"></i>{{__('Print')}}</span>
                </a>
            </button>
        </div>
    </div>
        ...
        <div class="col-md-12">
            <div class="card" id="printTable">
                <div class="card-header">
                    <h5 class="">{{ App\Models\Utility::invoiceNumberFormat($invoice->invoice_id) }}</h5>   
                </div>
                                ...
                                <table class="table invoice-table ">
                                    <tbody>
                                        <tr>
                                            ...
                                            @endphp
                                            <th>{{__('Subtotal')}} :</th>
                                            <td>{{$users->priceFormat($subTotal)}}</td>
                                        </tr>
                                        <tr>
                                            <th>{{__('Discount')}} :</th>
                                            <td>{{$users->priceFormat($invoice->discount)}}</td>
                                        </tr>
                                        <tr>
                                            <th>{{(!empty($invoice->tax)?$invoice->tax->name:'Tax')}} ({{(!empty($invoice->tax)?$invoice->tax->rate:'0')}} %) :</th>
                                            <td>{{$users->priceFormat($tax)}}</td>
                                        </tr>
                                        <tr>
                                            <th>{{__('Due Amount')}} :</th>
                                            <td>{{$users->priceFormat($invoice->getDue())}}</td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <hr/>
                                            ...
                                            <td>
                                                <hr/>
                                                <h5 class="text-primary">{{$users->priceFormat($subTotal-$invoice->discount+$tax)}}</h5>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
