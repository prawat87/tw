...
@section('breadcrumb')
    <li class="breadcrumb-item" aria-current="page"><a href="{{route('contracts.index')}}">{{__('Contract')}}</a></li>
...
<div class="col-md-6 text-end d-flex ">
    ...
    @endif
    @if(\Auth::user()->type=='company' && $contract->status == 'accept')
        <a href="#" class="btn btn-sm btn-primary btn-icon m-1" data-url="{{ route('contracts.copy',$contract->id) }}" data-ajax-popup="true" data-title="{{__('Duplicate Contract')}}" data-size="lg" title="{{__('Duplicate')}}" data-bs-toggle="tooltip" data-bs-placement="top">
            <i class="ti ti-files"></i>
        </a>
    @endif
    @if(\Auth::user()->type=='company' || \Auth::user()->type=='client')
        <a href="{{route('contract.download.pdf',\Crypt::encrypt($contract->id))}}" class="btn btn-sm btn-primary btn-icon m-1" data-bs-toggle="tooltip" data-bs-placement="top" title="{{__('Download')}}" target="_blanks"><i class="ti ti-download"></i></a>
    @endif
    @if(\Auth::user()->type=='company' || \Auth::user()->type=='client')
    ...
    @endif
    @if(((\Auth::user()->type =='company') && ($contract->company_signature == '')||(\Auth::user()->type =='client') && ($contract->client_signature == ''))&&($contract->status=='accept'))
        <a href="#" class="btn btn-sm btn-primary btn-icon m-1" data-url="{{ route('signature',$contract->id) }}" data-ajax-popup="true" data-title="{{__('create signature')}}" data-size="md" title="{{__('Signature')}}" data-bs-toggle="tooltip" data-bs-placement="top">
            <i class="ti ti-writing-sign"></i>
        </a>
    @endif

    @php
    $status = App\Models\Contracts::status();
    @endphp
       <ul class="list-unstyled mb-0 ms-1 m-1 drp-languages">
                                    ...
                                    <div class="card-body pt-0">
                                        ...
                                        <address class="mb-0 text-sm">
                                            <div class="row mt-4 align-items-center">
                                                <div class="col-sm-4 h6 text-sm">{{ __('Name') }}</div>
                                                <div class="col-sm-8 text-sm"> {{  $contract->clients->name }}</div>

                                                <div class="col-sm-4 h6 text-sm">{{ __('Project') }}</div>
                                                <div class="col-sm-8 text-sm">{{!empty( $contract->projectss)?$contract->projectss->name:''}}</div>
                                        ...
                                        <div class="card-footer">
                                            ...
                                            <div class="col-12 d-flex">
                                                <div class="form-group mb-0 form-send w-100">
                                                    <form method="post" class="card-comment-box" id="form-comment" data-action="{{route('comment_store.store',$contract->id)}}">

                                                        <input type="hidden" id="formCommentUrl" value="{{route('comment_store.store',$contract->id)}}">
                                            ...
                                            <div class="list-group-item ">
                                                <div class="row align-items-center">
                                                    <div class="col-auto">
                                                        ...
                                                        <img src="{{(!empty($user->avatar))?  \App\Models\Utility::get_file($user->avatar): $logo."/avatar.png"}}" class="rounded-circle" width="25" height="25">
                                                    </div>
                                                    <div class="col ml-n2">
                                                    ...
                                                    @if(((\Auth::user()->id == $comment->user_id) || (\Auth::user()->type == 'company')) &&($contract->status == 'accept'))
                                                        <div class="col-auto">
                                                            <a href="#" class="action-btn btn-danger mx-1 mt-1 btn btn-sm d-inline-flex align-items-center bs-pass-para" data-confirm="{{__('Are You Sure?')}}" data-text="{{__('This action can not be undone. Do you want to continue?')}}" data-confirm-yes="delete-form-{{$comment->id}}" title="{{__('Delete')}}" data-bs-toggle="tooltip" data-bs-placement="top"><span class="text-white"><i class="ti ti-trash"></i></span></a>
            
                                                            {!! Form::open(['method' => 'GET', 'route' => ['comment_store.destroy', $comment->id], 'id' => 'delete-form-' . $comment->id]) !!}
                                                            {!! Form::close() !!}
                                                        </div>
                                                    @endif
                                                </div>
                                                    ...
                                                    <div class="list-group-item ">
                                                        <div class="row align-items-center">
                                                            ...
                                                            @if(((\Auth::user()->id == $note->user_id) || (\Auth::user()->type == 'company'))&&($contract->status == 'accept'))
                                                                <a href="#" class="action-btn btn-danger mx-1 mt-1 btn btn-sm d-inline-flex align-items-center bs-pass-para" data-confirm="{{__('Are You Sure?')}}" data-text="{{__('This action can not be undone. Do you want to continue?')}}" data-confirm-yes="delete-form-{{$note->id}}" title="{{__('Delete')}}" data-bs-toggle="tooltip" data-bs-placement="top"><span class="text-white"><i class="ti ti-trash"></i></span></a>
                        
                                                                {!! Form::open(['method' => 'GET', 'route' => ['note_store.destroy', $note->id], 'id' => 'delete-form-' . $note->id]) !!}
                                                                {!! Form::close() !!}
                                                            @endif
                                                        </div>
                                                    </div>
...
<script>
    $(document).on('click', '#comment_submit', function (e) 
    {
        ...
        var actionUrl = $('#formCommentUrl').val();
        if (comment != '') {
            $.ajax({
                url: actionUrl,
                ...
                success: function (data) {
                    ...
                    show_toastr('{{__("Success")}}', 'Comment Added Successfully!', 'success');
                    setTimeout(function () {
                        location.reload();
                    }, 500)
