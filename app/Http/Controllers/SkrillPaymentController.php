...
class SkrillPaymentController extends Controller
{
    ...
    public function invoicePayWithSkrill(Request $request)
    {
        ...
        $skill->prepare_only    = '1';
        $skill->merchant_fields = 'site_name, customer_email';
        $skill->site_name       = $users->name;
        $skill->customer_email  = $users->email;
    ...
    public function getInvociePaymentStatus(Request $request,$invoice_id)
    {
        if(!empty($invoice_id))
        {
            ...
            if($invoice)
            {
                ...
                catch(\Exception $e)
                {
                    ...
                    {
                        return redirect()->route('pay.invoice',\Illuminate\Support\Facades\Crypt::encrypt($invoices->id))->with('error', __('Invoice not found.'));
                    }
                }
        ...
        else
        {
            ...
            {
                return redirect()->route('pay.invoice',\Illuminate\Support\Facades\Crypt::encrypt($invoice->id))->with
